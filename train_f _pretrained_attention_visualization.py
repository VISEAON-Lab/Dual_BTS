...
from Siamese_nt.TransGeo_attention_visualization import TransGeo
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    #quarter_query_datatset = torchvision.datasets.ImageFolder(root="imgs/")
    quarter_query_datatset = dataset(mode='quater_query' , args=args)
    quarter_reference_dataset = dataset(mode='quater_reference', args=args)
    ...
    quater_query_loader = torch.utils.data.DataLoader(
        quarter_query_datatset, batch_size=4, shuffle=False,
        num_workers=args.workers, pin_memory=True)  # 512, 64
    quater_reference_loader = torch.utils.data.DataLoader(
        quarter_reference_dataset, batch_size=4, shuffle=False,
        num_workers=args.workers, pin_memory=True)  # 80, 128
    ...
    if not args.crop:
        ...
        # model.query_net.save = os.path.join(args.save_path, 'attention', 'trainQ')
        # scan(quater_query_loader, model, args)
        # #model.reference_net.save = os.path.join(args.save_path, 'attention', 'val')
        # #scan(val_scan_loader, model, args)

        model.query_net.save = os.path.join(args.save_path, 'attention', 'train')
        scan(train_scan_loader, model, args)#train_scan_loader
        # model.reference_net.save = os.path.join(args.save_path, 'attention', 'val')
