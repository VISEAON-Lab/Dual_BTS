...
def best_5(val_query_loader, val_reference_loader, model, args,test_id):
    ...
    for j in range(len(test_id)):
        ...
        plt.axis('off')
        # img = Image.open(test_id[j][2].replace('\n', ""))
        img = cv2.imread(
        ...
        img = img.resize((224, 224))
        n = 0
        for h in range(0, 197,28):#(0, 196, 28)
            area = (h,0, h+28,224)  #MM( 56,h, 168,h+56), ML( 1,h, 112,h+56), MR(112,h, 224,h+56)# LM(1,56,112,168),RM(112,56,224,168)
            cropped_img = img.crop(area)
            cropped_img.save(os.path.join(r"D:\fabian\PycharmProjects\transgeo1\results\quarters\flc\flc",
                                               (str(n) + ".jpg")))
            # cropped_img.show()
            n += 1
        n = 0
        ...
        img = Image.fromarray(img)
        img = img.resize((224, 224))
        for h in range(0, 197, 28):
            area = (h,0, h+28,224)#MM( 56,h, 168,h+56), ML( 1,h, 112,h+56), MR(112,h, 224,h+56)
            cropped_img = img.crop(area)
            cropped_img.save(os.path.join(r"D:\fabian\PycharmProjects\transgeo1\results\quarters\sonar\sonar", (str(n) + ".jpg")))
            #cropped_img.show()
            n+=1
